pub fn generate(default_wl: String) -> String {
    format!(
        ...
        const currentWhitelabel = (() => {{
          // Fallback to '{}' (or your project's default) if the env var is missing
          const variant = (process.env.NEXT_PUBLIC_WHITELABEL as Variants) || '{}';
         ...
         * const variant = host?.includes('brand-a') ? 'brandA' : '{}';
         * return new Whitelabel()[variant as Variants];
         * }};
         * ============================================================================
         */"#,
        default_wl, default_wl, default_wl,
    )
    .to_string()
}
